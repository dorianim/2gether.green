...
export interface OwnDialogProps {
  ...
  onClose: (total_cost: number,
    revenue_per_month: number,
    construction_time: number) => void;
}

export default function OwnDialog(props: OwnDialogProps) {
  const [total_cost, set_total_cost] = React.useState(props.total_cost);
  const [construction_time, set_construction_time] = React.useState(props.construction_time);
  const [revenue_per_month, set_revenue_per_month] = React.useState(props.revenue_per_month);
